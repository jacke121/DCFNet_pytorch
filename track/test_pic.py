...
if __name__ == '__main__':
    ...
    init_rect =[900,263,180,70]# np.array(annos[video]['init_rect']).astype(np.float)
    image_files = [base_path+f for f in os.listdir(base_path)]
    ...
    for f in range(1, n_images):  # track
        ...
        fps = 1 / toc
        speed.append(fps)
        print('Time: {:3.1f}s\tSpeed: {:3.1f}fps'.format(toc, fps))
